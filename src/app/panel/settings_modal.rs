    ...
    pub(in crate::app) fn draw_settings_modal(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Settings")
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new("settings_unit_labels")
                    .num_columns(2)
                    .spacing([8.0, 4.0])
                    .show(ui, |ui| {
                        let lw = 40.0_f32;
                        ...
                        ui.add_sized(egui::vec2(lw, 18.0),
                            egui::Label::new(RichText::new("mass").size(10.0).color(TEXT_SEC)));
                        ui.add(egui::TextEdit::singleline(&mut self.physics_cfg.mass_label)
                            .desired_width(fw));
                        ui.end_row();

                        ui.add_sized(egui::vec2(lw, 18.0),
                            egui::Label::new(RichText::new("dist").size(10.0).color(TEXT_SEC)));
                        ui.add(egui::TextEdit::singleline(&mut self.physics_cfg.dist_label)
                            .desired_width(fw));
                        ui.end_row();

                        ui.add_sized(egui::vec2(lw, 18.0),
                            egui::Label::new(RichText::new("time").size(10.0).color(TEXT_SEC)));
                        ui.add(egui::TextEdit::singleline(&mut self.physics_cfg.time_label)
                            .desired_width(fw));
                        ui.end_row();
                    });
                ...
                egui::Grid::new("settings_recording")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label(RichText::new("Δt_record").size(10.0).color(TEXT_SEC))
                            .on_hover_text("Simulated time gap between CSV rows.\nRule of thumb: 10–100 × dt.");
                ...
                if is_recording {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.label(RichText::new(format!("— {records} rows")).size(10.0).color(TEXT_DIM));
                    });
                    ui.label(
                    ...
                    ui.add_space(6.0);
                    if ui.add(
                        egui::Button::new(RichText::new("■  Stop recording").size(10.5).color(DANGER))
                    ...
                    if ui.add(
                        egui::Button::new(RichText::new("●  Start recording").size(10.5).color(SUCCESS))
