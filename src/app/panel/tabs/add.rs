    ...
    egui::Grid::new(ui.id().with("mat_grid"))
        ...
        .show(ui, |ui| {
            for (i, &mat) in Material::ALL.iter().enumerate() {
                ...
                let btn = ui.add(
                    egui::Button::new(
                        egui::RichText::new(mat.display_name())
                            .size(9.5)
...
fn drag_row(ui: &mut egui::Ui, label: &str, hint: &str, widget: egui::DragValue) {
    ui.horizontal(|ui| {
        ui.label(RichText::new(label).size(10.5).color(TEXT_SEC))
            .on_hover_text(hint);
...
impl SimulationApp {
    ...
    fn spawn_sub_tab_bar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            if tab_btn(ui, SpawnTab::Single,  "Single")  { self.spawn_tab = SpawnTab::Single;  }
            if tab_btn(ui, SpawnTab::Ring,    "Ring")    { self.spawn_tab = SpawnTab::Ring;    }
            if tab_btn(ui, SpawnTab::Cluster, "Cluster") { self.spawn_tab = SpawnTab::Cluster; }
        });
    }
    ...
    fn panel_add_single(&mut self, ui: &mut egui::Ui) {
        ...
        drag_row(ui, "mass", "Body mass in simulation units",
        ...
        drag_row(ui, "density", "Bulk density (controls physical radius)",
        ...
        if self.place_mode {
            ui.add_space(3.0);
            ui.label(
                RichText::new("drag to set initial velocity")
                    .size(9.0)
                    .color(TEXT_DIM)
                    .italics(),
            );
        ...
        egui::CollapsingHeader::new(
            RichText::new("Manual coordinates").size(10.0).color(TEXT_SEC),
        )
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(
                        RichText::new(&r_preview).monospace().size(9.5).color(TEXT_SEC),
                    );
                });
            });
    ...
    fn panel_add_ring(&mut self, ui: &mut egui::Ui) {
        ...
        drag_row(ui, "radius", "Ring radius around the system's centre of mass",
            egui::DragValue::new(&mut self.spawn_ring_radius)
                .speed(ring_r_speed)
                .range(0.01..=1e6_f64)
                .max_decimals(4),
        );
        drag_row(ui, "count", "Number of bodies evenly spaced around the ring",
            egui::DragValue::new(&mut self.spawn_ring_count)
                .speed(1.0)
                .range(2..=10_000u32),
        );
        drag_row(ui, "mass each", "Mass of each individual body",
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                info_row(ui, "v_ring",   &format!("{:.4e}  (×{:.2})", v_ring, self.spawn_ring_vel_scale));
                info_row(ui, "COM",      &format!("({:.3}, {:.3})", m.com_x, m.com_y));
                info_row(ui, "total M",  &format!("{:.4e}",
                    self.spawn_ring_mass * self.spawn_ring_count as f64));
            });
        ...
        if primary_btn(ui, &format!("+ Add ring  ({} bodies)", self.spawn_ring_count)) {
            ...
            if n > 0 && self.spawn_ring_radius > 0.0 {
                ...
                for i in 0..n {
                    ...
                    let vx = -v * angle.sin();
                    let vy =  v * angle.cos();
                    let mut b = Body::new(x, y, vx, vy, self.spawn_ring_mass, self.spawn_ring_material);
    ...
    fn panel_add_cluster(&mut self, ui: &mut egui::Ui) {
        ...
        drag_row(ui, "radius", "Half-mass radius of the cluster",
            egui::DragValue::new(&mut self.spawn_cluster_radius)
                .speed(clust_r_speed)
                .range(0.01..=1e6_f64)
                .max_decimals(4),
        );
        drag_row(ui, "count", "Number of bodies",
            egui::DragValue::new(&mut self.spawn_cluster_count)
                .speed(1.0)
                .range(1..=50_000u32),
        );
        drag_row(ui, "mass each", "Mass of each body",
        ...
        let kinetic_est = self.spawn_cluster_count as f64 * 1.5
            * self.spawn_cluster_mass * sigma2;
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                info_row(ui, "σ_3D",     &format!("{:.4e}", (3.0_f64).sqrt() * self.spawn_cluster_vel_disp));
                ui.horizontal(|ui| {
        ...
        if primary_btn(ui, &format!("+ Add cluster  ({} bodies)", self.spawn_cluster_count)) {
            ...
            if n > 0 {
                ...
                for _ in 0..n {
                    ...
                    let mut b = Body::new(x, y, vx, vy, self.spawn_cluster_mass, self.spawn_cluster_material);
