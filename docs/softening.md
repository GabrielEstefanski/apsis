...
Implementation: `src/physics/gravity/kernel.rs::pair_eps2`.
...
Implementation: `src/domain/body.rs::default_softening`.
...
| File | Role |
|------|------|
| `src/domain/body.rs` | `EPS_BASE`, `default_softening()`, `Body::softening` field |
| `src/physics/gravity/kernel.rs` | `pair_eps2()`, `plummer_acc()`, `plummer_phi()` |
| `src/physics/gravity/engine.rs` | Per-body softening used in BH and exact evaluations |
| `src/core/system.rs` | `set_softening_scale()`, `compute_closeness()` (r_min tracking) |
| `src/core/metrics.rs` | `Metrics::r_min`, `Metrics::softening_max` |
| `src/app/panel/tabs/config.rs` | ε scale slider + real-time validity indicator |
