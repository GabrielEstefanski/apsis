//! Perturbation transport — extracts a `Box<dyn HamiltonianOperator>`
//! out of an `apsis.Perturbation` Python object's underlying `PyCapsule`.
//!
//! The user-facing `Perturbation` class is defined in
//! `python/apsis/__init__.py` (pure Python). This module is just the
//! `add_hamiltonian_perturbation` boundary: pull the capsule attribute,
//! hand it to [`apsis_py_core::take_box_from_capsule`], forward the
//! boxed trait object to the core. See `apsis-py-core` for the
//! rationale on why the wrapper is pure-Python rather than a pyclass.
...
/// Pull the boxed operator out of a Python object the user passed to
/// `System.add_hamiltonian_perturbation`. Accepts anything carrying a
