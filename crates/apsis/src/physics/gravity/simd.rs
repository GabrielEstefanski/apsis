//! SIMD intrinsic kernels for the Barnes-Hut walk.
//!
//! Vectorises the leaf-pair phase of the two-phase walk
//! ([`bh_process_lists`](super::engine::bh_process_lists)) using
//! `std::arch::x86_64` AVX2 intrinsics. The accepted-node phase remains
//! scalar — vectorising it across nodes would require either gather
//! across non-contiguous `Node` memory (~10-15 cycles per gather × 9
//! fields per node, slower than the ~60-80 cycle scalar interaction)
//! or a staging buffer (~15k writes per body during walk, doubling walk
//! cost). Both paths net-negative on the recorded hardware. Out of scope
//! deferred to PR-perf-7 if a future profile makes accepted-node SIMD
//! warranted.
//!
//! ## Tier 0 microbenchmarks
//!
//! [`saxpy_scalar`] / [`saxpy_avx2`] are the trivial-workload sanity
//! check for the toolchain + CPU SIMD throughput, called by the
//! `perf_simd` harness's Tier 0 gate.
//!
//! ## Layout assumptions
//!
//! `BodyArrays` is SoA puro (5 separate `Vec<f64>`); leaf-mate body
//! indices are scattered (not Morton-sorted), so the AVX2 kernel uses
//! `_mm256_i32gather_pd` for each field load. Aligned-load alternative
//! would require AoSoA + Morton reordering, deferred to a conditional
//! PR-perf-7 per the §Decision rules of the SIMD lab notebook.
...
// ── AVX-512 mirrors (8 doubles per register) ───────────────────────────────── //

/// AVX-512 saxpy via `_mm512_fmadd_pd` (8 doubles per FMA instruction).
///
/// Caller must ensure AVX-512F is available.
/// Tier 0 gate predicts `t_scalar / t_avx512 ≥ 4.0×` on Zen 4 (lane count
/// doubles vs AVX2; Zen 4 implements AVX-512 via two AVX2 µops in some
/// cases, so the realistic ratio over scalar is closer to 4× than 8×).
#[target_feature(enable = "avx512f")]
pub(crate) unsafe fn saxpy_avx512(a: f64, x: &[f64], y: &mut [f64]) {
    let n = x.len().min(y.len());
    let n_chunks = n / 8;

    unsafe {
        let a_vec = _mm512_set1_pd(a);

        for chunk in 0..n_chunks {
            let off = chunk * 8;
            let xv = _mm512_loadu_pd(x.as_ptr().add(off));
            let yv = _mm512_loadu_pd(y.as_ptr().add(off));
            let res = _mm512_fmadd_pd(a_vec, xv, yv);
            _mm512_storeu_pd(y.as_mut_ptr().add(off), res);
        }
    }
...
/// AVX-512 Plummer kernel microbench, 8 interactions per chunk via
/// aligned SIMD ops on pre-laid-out arrays. Uses `_mm512_reduce_add_pd`
/// for the horizontal sum (built-in cross-lane reduction).
#[target_feature(enable = "avx512f")]
pub(crate) unsafe fn plummer_kernel_avx512_micro(
    target_acc: &mut Vec3,
    dx: &[f64],
    dy: &[f64],
    dz: &[f64],
    ...
    let n = dx.len();
    let n_chunks = n / 8;
    debug_assert_eq!(n % 8, 0, "microbench requires N divisible by 8");
    debug_assert_eq!(dy.len(), n);
    debug_assert_eq!(dz.len(), n);
    debug_assert_eq!(eps2.len(), n);
    debug_assert_eq!(other_mass.len(), n);
    ...
    unsafe {
        ...
        let mut ax_acc = _mm512_setzero_pd();
        let mut ay_acc = _mm512_setzero_pd();
        let mut az_acc = _mm512_setzero_pd();

        for chunk in 0..n_chunks {
            let off = chunk * 8;
            let dx_v = _mm512_loadu_pd(dx.as_ptr().add(off));
            let dy_v = _mm512_loadu_pd(dy.as_ptr().add(off));
            let dz_v = _mm512_loadu_pd(dz.as_ptr().add(off));
            let eps2_v = _mm512_loadu_pd(eps2.as_ptr().add(off));
            let mass_v = _mm512_loadu_pd(other_mass.as_ptr().add(off));

            let dx_sq = _mm512_mul_pd(dx_v, dx_v);
            let dy_sq = _mm512_mul_pd(dy_v, dy_v);
            let dz_sq = _mm512_mul_pd(dz_v, dz_v);
            let r_sq = _mm512_add_pd(_mm512_add_pd(dx_sq, dy_sq), dz_sq);

            let r_sq_eff = _mm512_add_pd(r_sq, eps2_v);
            let r_eff = _mm512_sqrt_pd(r_sq_eff);
            let inv_r = _mm512_div_pd(one, r_eff);

            let inv_r2 = _mm512_mul_pd(inv_r, inv_r);
            let inv_r3 = _mm512_mul_pd(inv_r2, inv_r);

            let g_mass = _mm512_mul_pd(g_vec, mass_v);
            let fac = _mm512_mul_pd(g_mass, inv_r3);

            ax_acc = _mm512_fmadd_pd(dx_v, fac, ax_acc);
            ay_acc = _mm512_fmadd_pd(dy_v, fac, ay_acc);
            az_acc = _mm512_fmadd_pd(dz_v, fac, az_acc);
        }

        target_acc.x += _mm512_reduce_add_pd(ax_acc);
        target_acc.y += _mm512_reduce_add_pd(ay_acc);
        target_acc.z += _mm512_reduce_add_pd(az_acc);
    }
}

/// AVX-512 production leaf-pair kernel — 8-lane gather + FMA mirror of
/// [`process_leafpair_avx2`]. Loads scattered fields via
/// `_mm512_i32gather_pd` (8 i32 indices per vector). Caller must ensure
/// AVX-512F is available.
#[target_feature(enable = "avx512f")]
pub(crate) unsafe fn process_leafpair_avx512(
    body_pos_x: f64,
    body_pos_y: f64,
    body_pos_z: f64,
    body_softening: f64,
    ...
    let (mut a, mut phi) = unsafe {
        let bpx = _mm512_set1_pd(body_pos_x);
        let bpy = _mm512_set1_pd(body_pos_y);
        let bpz = _mm512_set1_pd(body_pos_z);
        let body_soft_sq = _mm512_set1_pd(body_softening * body_softening);
        let g_vec = _mm512_set1_pd(G);
        let half = _mm512_set1_pd(0.5);
        let one = _mm512_set1_pd(1.0);
        let neg_g = _mm512_set1_pd(-G);

        let mut ax_acc = _mm512_setzero_pd();
        let mut ay_acc = _mm512_setzero_pd();
        let mut az_acc = _mm512_setzero_pd();
        let mut phi_acc = _mm512_setzero_pd();

        for chunk in 0..n_chunks {
            let off = chunk * 8;
            let idx = _mm256_loadu_si256(leaf_body_indices.as_ptr().add(off) as *const __m256i);

            let opx = _mm512_i32gather_pd::<8>(idx, arrays.pos_x.as_ptr());
            let opy = _mm512_i32gather_pd::<8>(idx, arrays.pos_y.as_ptr());
            let opz = _mm512_i32gather_pd::<8>(idx, arrays.pos_z.as_ptr());
            let omass = _mm512_i32gather_pd::<8>(idx, arrays.mass.as_ptr());
            let osoft = _mm512_i32gather_pd::<8>(idx, arrays.softening.as_ptr());

            let dx = _mm512_sub_pd(opx, bpx);
            let dy = _mm512_sub_pd(opy, bpy);
            let dz = _mm512_sub_pd(opz, bpz);

            let r_sq = _mm512_add_pd(
                _mm512_add_pd(_mm512_mul_pd(dx, dx), _mm512_mul_pd(dy, dy)),
                _mm512_mul_pd(dz, dz),
            );

            let other_soft_sq = _mm512_mul_pd(osoft, osoft);
            let pair_eps2 = _mm512_mul_pd(_mm512_add_pd(body_soft_sq, other_soft_sq), half);

            let r_sq_eff = _mm512_add_pd(r_sq, pair_eps2);
            let r_eff = _mm512_sqrt_pd(r_sq_eff);
            let inv_r = _mm512_div_pd(one, r_eff);
            ...
            let g_mass = _mm512_mul_pd(g_vec, omass);
            let fac = _mm512_mul_pd(g_mass, inv_r3);

            ax_acc = _mm512_fmadd_pd(dx, fac, ax_acc);
            ay_acc = _mm512_fmadd_pd(dy, fac, ay_acc);
            az_acc = _mm512_fmadd_pd(dz, fac, az_acc);

            let neg_g_mass = _mm512_mul_pd(neg_g, omass);
            phi_acc = _mm512_fmadd_pd(neg_g_mass, inv_r, phi_acc);
        }

        (
            Vec3::new(
                _mm512_reduce_add_pd(ax_acc),
                _mm512_reduce_add_pd(ay_acc),
                _mm512_reduce_add_pd(az_acc),
            ),
            _mm512_reduce_add_pd(phi_acc),
        )
    };

    for &raw_bi in &leaf_body_indices[n_chunks * 8..] {
        let bi = raw_bi as usize;
        let other_mass = arrays.mass[bi];
        let dx_s = arrays.pos_x[bi] - body_pos_x;
        let dy_s = arrays.pos_y[bi] - body_pos_y;
