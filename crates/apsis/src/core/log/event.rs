...
impl Event {
    ...
    /// format: `[apsis <TAG>] <message> { key1=value1 key2=value2 }`.
    /// Pre-existing log scrapers depend on the prefix token.
    pub fn format_single_line(&self) -> String {
        if self.fields.is_empty() {
            format!("[apsis {}] {}", self.level.tag(), self.message)
        } else {
            use std::fmt::Write as _;
            let mut s = String::with_capacity(128);
            let _ = write!(&mut s, "[apsis {}] {} {{", self.level.tag(), self.message);
...
mod tests {
    ...
    fn format_single_line_with_fields() {
        ...
        let s = e.format_single_line();
        assert_eq!(s, "[apsis WARN] floor reached { dt=1e-12 count=42 }");
    }
