"""Orchestrator for REBOUND parity — Retrograde Kepler e=0.5.

Runs the apsis side (Cargo example), then the REBOUND side (Python), then
the comparator. Exits with the comparator's exit code.
...
- 0 — all metrics within tolerance at both horizons.
- 1 — input file error in the comparator.
- 2 — at least one metric exceeded tolerance at either horizon.
...
def main() -> int:
    ...
    print("=" * 72)
    print("[1/3] apsis IAS15 (Cargo example) — 10000 orbits, retrograde")
    print("=" * 72)
    rc = subprocess.call(
        [
            "cargo", "run", "--release",
            "--example", "rebound_parity_retrograde",
            "-p", "apsis",
            "--",
    ...
    print()
    print("=" * 72)
    print("[2/3] REBOUND IAS15 (Python) — 10000 orbits, retrograde")
    print("=" * 72)
    rc = subprocess.call(
        [
    ...
    print()
    print("=" * 72)
    print("[3/3] Comparator (two-horizon: 100-orbit checkpoint + 10000-orbit gate)")
    print("=" * 72)
    rc = subprocess.call(
        [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the full apsis/REBOUND parity comparison for retrograde Kepler e=0.5.",
    )
    parser.add_argument(
        "--workspace-root",
