"""Orchestrator for REBOUND parity — Kepler e=0.5.
...
# scenario dir is `<root>/validation/rebound-parity/kepler`, so the
...
def main() -> int:
    ...
    print("=" * 72)
    print("[1/3] apsis IAS15 (Cargo example)")
    print("=" * 72)
    rc = subprocess.call(
        [
            "cargo", "run", "--release",
            "--example", "rebound_parity_kepler",
            "-p", "apsis",
            "--",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the full apsis/REBOUND parity comparison for Kepler e=0.5.",
    )
    parser.add_argument(
        "--workspace-root",
